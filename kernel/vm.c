...
void printtbl(pagetable_t pagetable, int level)
{
  for (int i = 0; i < 512; i++)
  {
    ...
    if (pte & PTE_V)
    {
      printdoubledots(level);
      printf("%d: pte %p pa %p\n", i, pte, PTE2PA(pte));

      if ((pte & PTE_V) && (pte & (PTE_R | PTE_W | PTE_X)) == 0)
      {
        uint64 child = PTE2PA(pte);
        printtbl((pagetable_t)child, level + 1);
      }
    }
  }
}
...
{
  // printf("begin uvmfreewithoutleaf : %d\n", sz);
  // if(sz > 0)
    // uvmunmap(pagetable, 0, PGROUNDUP(sz)/PGSIZE, 1);
  kernelfreewalk(pagetable);
    // printf("end uvmfreewithoutleaf\n");
}

void
kuvmcopy(pagetable_t old, pagetable_t new, uint64 start, uint64 end)
{
  pte_t *old_pte;
  pte_t *new_pte;
